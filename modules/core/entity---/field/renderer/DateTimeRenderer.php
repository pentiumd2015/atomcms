...
class DateTimeRenderer extends Renderer{
    ...
    public function renderDetail($value, $arValues, $arParams = array()){
        $obField        = $this->getField();
        $arFieldParams  = $obField->getParams();
        $pk             = $obField->getEntity()->getPk();

        if($arFieldParams["disabled"] && !isset($arValues[$pk])){ //если добавляем новую запись и поле отключено, то ничего отображать не надо
            return "";
        }
