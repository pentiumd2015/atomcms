...
<div class="page-header">
    <div class="page-title">
        <h3><?=$obEntity->title;?>, поле: <?=$arFormData["title"];?><small>Редактирование поля</small></h3>
    </div>
</div>
<?
    if(count($arErrors)){
        ?>
            <div class="callout callout-danger">
                ...
                <h5>Ошибка сохранения поля</h5>
                <p>При сохранении поля возникли ошибки.</p>
            </div>
        <?
    }
?>
<form class="form-horizontal entity_field_form" method="POST" action="<?=$editURL;?>">
    <?=CHtml::hidden("entity_field[entity_id]", $obEntity->entity_id);?>
    <div class="panel panel-default">
        <div class="panel-heading">
            <h6 class="panel-title">Поле: <?=$arFormData["title"];?></h6>
        </div>
...
$(function(){
    function refreshFieldParams(){
        ...
        data = $.extend({
            ...
            entityFieldID   : "<?=$arFormData["entity_field_id"];?>"
        }, data);
