            ...
            <?
                if($obEntity->use_sections){
                    ?>
                        ...
                        <a class="btn btn-primary" href="<?=$listSectionURL;?>">
                            <i class="icon-tree2"></i>
                            <?=$obEntity->params["signatures"][Entity::SIGNATURE_SECTIONS]["title"];?>
                        </a>
                    <?
                }
            ?>
...
\Helpers\CBuffer::start();
    ...
    if(count($arEntityElements)){
        foreach($arEntityElements AS $obEntityElement){
            $itemURL = str_replace("{ID}", $obEntityElement->entity_item_id, $editElementURL);
            ?>
                <tr data-id="<?=$obEntityElement->entity_item_id;?>">
                    <td class="text-center">
                        <div class="checkbox checkbox-primary">
                            <?=CHtml::checkbox("checkbox_item[]", false, array(
                                "id"    => "checkbox_item",
                                "value" => $obEntityElement->entity_item_id
                            ));?>
