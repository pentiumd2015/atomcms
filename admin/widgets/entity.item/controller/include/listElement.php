...
if($_REQUEST["group"] && $_REQUEST["checkbox_item"]){
    ...
    if(is_array($arGroupItems)){
        switch($_REQUEST["group"]){
            case "activate":
                EntityElement::updateAllByPk($arGroupItems, array("active" => 1));
                break;
            case "deactivate":
                EntityElement::updateAllByPk($arGroupItems, array("active" => 0));
                break;
            case "delete":
                EntityElement::deleteAllByPk($arGroupItems);
                break;
        }
    }
}
...
if($_REQUEST["sort"]){
    ...
    if(substr($sortField, 0, 2) == "f_"){
        ...
        $obFieldType = new FieldTypeBase($obEntity, EntityItem::TYPE_ELEMENT);
        $arSqlParams = $obFieldType->getSqlParams($arSqlParams, array(
            "fieldName" => $sortField,
            "by"        => $sortBy
        ), "list.sort");
...
$arEntityElements   = EntityElement::findAll($arSqlParams["params"], $arSqlParams["statements"]);
$arEntityElementIDs = CArrayHelper::getColumn($arEntityElements, "entity_item_id");
...
$arEntityDisplayList    = EntityAdminDisplay::getDisplayMap($obEntity->entity_id, $sectionID, $userID, EntityItem::TYPE_ELEMENT, "list");
$arEntityDisplayFilter  = EntityAdminDisplay::getDisplayMap($obEntity->entity_id, $sectionID, $userID, EntityItem::TYPE_ELEMENT, "filter");
/*entity display*/

/*Field Values*/
$arEntityFieldValues = array();

if(count($arEntityElementIDs)){
    $arTmpEntityFieldValues = EntityElementFieldValue::findAll(array(
        "condition" => "entity_item_id IN(" . implode(", ", $arEntityElementIDs) . ")",
...
$this->setData(array(
    ...
    "obEntity"              => $obEntity,
    "arEntityElements"      => $arEntityElements,
    ...
    "displaySettingsURL"    => "/admin/settings/entities/" . $obEntity->entity_id . "/display/?entity_display[relation]=" . EntityItem::TYPE_ELEMENT . "&entity_display[type]=list"
));

$this->includeView("listElement");
?>